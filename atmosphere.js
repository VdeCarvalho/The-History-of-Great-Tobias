(() => {
  ...
  // Bake the expensive blur once. Runtime only moves small cached shadow tiles.
  const shadows=Array.from({length:9},(_,i)=>{
    const c=document.createElement('canvas');c.width=160;c.height=140;const q=c.getContext('2d');q.translate(80,70);q.filter='blur(3px)';q.fillStyle='rgba(36,49,28,.18)';
    for(let j=0;j<7;j++){const seed=i*11+j,angle=random(seed+80)*Math.PI*2,x=Math.cos(angle)*random(seed+10)*48,y=Math.sin(angle)*random(seed+20)*38,size=10+random(seed+30)*15;
      q.save();q.translate(x,y);q.rotate(angle);q.beginPath();q.moveTo(-size,0);q.bezierCurveTo(-size*.2,-size*.65,size*.7,-size*.25,size,0);q.bezierCurveTo(size*.25,size*.55,-size*.4,size*.4,-size,0);q.fill();q.restore();}
    return c;
  });
  ...
  window.TobiasAtmosphere={draw(ctx,image,maskImage,w,h,t){
    ...
    for(const y of [350,820]){
      ...
      for(let i=0;i<9;i++){
        ...
        g.strokeStyle='rgba(36,49,28,.12)';g.lineWidth=2+random(i)*3;
        g.beginPath();g.moveTo(88,y+random(i+51)*100);g.quadraticCurveTo(cx-80,cy-30,cx,cy);g.stroke();
        g.drawImage(shadows[i],cx-80,cy-70);
      }
    }
